...
void LEDMatrix_Init(void)
{
    ...
    memset(led_framebuffer, 0, sizeof(led_framebuffer));
}
...
void LEDMatrix_ShowNextRow(void)
{
    ...
    i2c_write_blocking(i2c0, 0x40, data, 25, true);

    // Prepare next address 
    gpio_put_masked(0b111 << ROWSEL_A0, current_row << ROWSEL_A0);
    // Dummy write to send STOP condition
    data[0] = 0x00;
    i2c_write_blocking(i2c0, 0x70, data, 1, false);
    // Activate new row by latching
    gpio_put(ROWSEL_LATCH, 0);
    gpio_put(ROWSEL_LATCH, 1);
    gpio_put(ROWSEL_EN, 1);
}